<?php
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if (isset($_FILES["fileToUpload"]["name"]) != null) {
	        ...
	        $filePath = $path.$name;
	        // Create new SplFileObject
		    $myfile = new SplFileObject($filePath);
		    // Determine whether the end of file has been reached
			while (!$myfile->eof()) {
	    		$buffer = $myfile->current(); // Retrieve current line of file
	    		echo $buffer;
	    		$myfile->next(); // Moves ahead to the next line in the file.
			}
			exit;
		}
	}
